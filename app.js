...
function renderLedger(){
  ...
  for(const k of allKeys()){
    ...
    if(d>=p.start&&d<=p.end){
      ...
      tr.className='ledger-row type-'+(r.type||'');tr.dataset.date=k;
      tr.innerHTML=`<td>${k.slice(5)}（${['日','月','火','水','木','金','土'][d.getDay()]}）</td><td>${holidayFor(k).type}</td><td>${r.type||''}</td><td>${r.start||''}</td><td>${r.end||''}</td><td>${c.work.toFixed(2)}</td><td>${c.overtime.toFixed(2)}</td><td>${comp.toFixed(1)}</td><td><button type="button" class="edit-button" data-edit-date="${k}">編集</button></td>`;
      tr.addEventListener('click',e=>{if(!e.target.closest('button'))openRecord(k)});
      $('ledgerRows').appendChild(tr)
    }
  }
  document.querySelectorAll('[data-edit-date]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();openRecord(b.dataset.editDate)}));
}
...
function openRecord(k){
  editDate=k;
  const r=state.records[k]||{type:holidayFor(k).type==='勤務日'?'出勤':'公休'},p=periodForDate(parseIso(k));
  $('editDateLabel').textContent=`${k}　${p?p.label+'（'+p.range+'）':''}`;
  $('editType').value=r.type||'出勤';$('editStart').value=r.start||'';$('editEnd').value=r.end||'';$('editOut').value=r.out||'';$('editBack').value=r.back||'';$('editNote').value=r.note||'';
  $('editSaveMessage').textContent='';
  $('ledgerEditor').hidden=false;
  document.querySelectorAll('.ledger-row').forEach(tr=>tr.classList.toggle('selected',tr.dataset.date===k));
  requestAnimationFrame(()=>$('ledgerEditor').scrollIntoView({behavior:'smooth',block:'start'}));
}
function closeLedgerEditor(){
  $('ledgerEditor').hidden=true;editDate='';
  document.querySelectorAll('.ledger-row').forEach(tr=>tr.classList.remove('selected'))
}
function saveEdited(){
  if(!editDate){$('editSaveMessage').textContent='編集する日付が選択されていません。';return}
  state.records[editDate]={type:$('editType').value,start:$('editStart').value,end:$('editEnd').value,out:$('editOut').value,back:$('editBack').value,note:$('editNote').value,updatedAt:new Date().toISOString()};
  persist();renderDashboard();renderLedger();renderTodayMetrics();
  if(editDate===iso())loadTodayForm();
  $('editSaveMessage').textContent=`${editDate} を保存しました。`;
  $('ledgerEditor').hidden=false
}
function deleteEdited(){
  if(!editDate)return;
  if(confirm(`${editDate} の入力を削除しますか？`)){
    delete state.records[editDate];persist();renderDashboard();renderLedger();renderTodayMetrics();if(editDate===iso())loadTodayForm();
    $('editSaveMessage').textContent=`${editDate} の入力を削除しました。`;$('ledgerEditor').hidden=false
  }
}
